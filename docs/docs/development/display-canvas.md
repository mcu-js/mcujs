...
| `image.info`, `decodeJPEG`, `drawJPEG` (`host/bindings/image.c`, picojpeg baseline decoder) | The bounded `jpeg` reader supplies RGB blocks to the existing Canvas API in `examples/portable/jpeg/show.js`. Decoding owns no display or graphics handle. No general `drawImage` API is added. | The portable header-only `images/info.js` helper and completion-driven `images/slideshow.js` replace the maintained board image demo and PiZero slideshow. The portable `images/features.js` diagnostic now covers positioning, clipping and layered graphics; the seven old image-only board diagnostics are retired. Native and physical qualification are separate; see [JPEG limits and proof boundaries](binary-file-assets.md#baseline-jpeg-example). |
| Legacy BMP decode/draw: 16-bit RGB565, 24-bit BGR, 32-bit BGRA (alpha ignored) | `examples/portable/sd-bmp/show.js` reads bounded raw `fs` rows into Canvas: explicit RGB565 `BI_BITFIELDS`, and 24/32-bit `BI_RGB`, with a 40-byte DIB. The fourth 32-bit byte is ignored. | Implicit 16-bit `BI_RGB`, other masks, extended DIBs and public image-info remain outside this example. Header-only metadata is available through the portable `images/info.js` helper; it does not certify decoder support. Native decoders remain. See [formats and proof boundaries](binary-file-assets.md#image-example). |
...
| Sticky full-frame refresh; ePaper V2 full refresh and gated partial-refresh policy (`platform/esp32/main/canvas_display_*.c`) | Private adapter policy, including BUSY waits and panel sleep/power sequencing. No portable partial-refresh selector is promised. | Sticky `37e3771` now has a usable REPL and photograph-proven BMP/white/BMP plus two settled Night Ferry software restarts; see [dated qualification](mcujs-0.2-portable-api.md#sticky-hardware-evidence--2026-09-13). Long-term ghosting, partial refresh, further consumers and other panels remain separate. The V2 board was not tested in this run. Do not change bootloaders to complete this audit. |
