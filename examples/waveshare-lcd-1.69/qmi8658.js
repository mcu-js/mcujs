...
// 6-axis accelerometer/gyroscope for Waveshare RP2350-Touch-LCD-1.69
...
// Create IMU driver
// Options: { i2cBus, sda, scl, baudrate }
function createIMUDriver(options) {
  ...
  var addr = QMI8658_ADDR;
  
  // Write register
  function writeReg(reg, value) {
    I2C.write(i2cBus, addr, [reg, value]);
  }
  
  // Read register
  function readReg(reg) {
    I2C.write(i2cBus, addr, [reg]);
    var data = I2C.read(i2cBus, addr, 1);
    return data[0];
  }
  ...
  function readRegs(reg, len) {
    I2C.write(i2cBus, addr, [reg]);
    return I2C.read(i2cBus, addr, len);
  }
  ...
  function init() {
    ...
    writeReg(REG_CTRL2, 0x15);
    accScale = 2.0;
    
    // Configure CTRL3: Gyroscope +/-256dps, 470.7Hz ODR
    writeReg(REG_CTRL3, 0x45);
    gyrScale = 256.0;
