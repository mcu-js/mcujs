// st7789v2.js - ST7789V2 Display Driver for mcujs
// Pure JavaScript driver for the Waveshare RP2350-Touch-LCD-1.69
// 240x280 RGB565 rectangular IPS display

// Default pin configuration for waveshare_rp2350_touch_lcd_1.69
...
// ST7789 window offset (display RAM is 240x320, so 280 rows need offset)
// Center 280 rows in 320-row RAM: (320-280)/2 = 20
var COL_OFFSET = 0;
var ROW_OFFSET = 20;  // 20-pixel Y offset to center 280 in 320

// Create a ST7789V2 display driver
// Options: { spiBus, sck, mosi, miso, cs, dc, rst, bl, baudrate, horizontal }
function createST7789V2Driver(options) {
  options = options || {};
  ...
  if (horizontal) {
    width = LCD_HEIGHT;   // 280
    height = LCD_WIDTH;   // 240
    colOffset = ROW_OFFSET;
    rowOffset = 0;
  } else {
    width = LCD_WIDTH;    // 240
    height = LCD_HEIGHT;  // 280
  ...
  function init() {
    ...
    writeReg(0xE0, [0xD0, 0x04, 0x0D, 0x11, 0x13, 0x2B, 0x3F, 0x54, 0x4C, 0x18, 0x0D, 0x0B, 0x1F, 0x23]);
    
    // Negative voltage gamma control
    writeReg(0xE1, [0xD0, 0x04, 0x0C, 0x11, 0x13, 0x2C, 0x3F, 0x44, 0x51, 0x2F, 0x1F, 0x1F, 0x20, 0x23]);
...
module.exports = {
  createST7789V2Driver: createST7789V2Driver,
