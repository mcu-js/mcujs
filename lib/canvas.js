...
// Installed as /lib/canvas.js. Hardware and presentation stay inside the native module.
...
Object.defineProperties(canvas, {
  width: { enumerable: true, value: native.width },
  height: { enumerable: true, value: native.height }
});
...
context.fill = function (fillRule) {
  ...
  if (path.length) native.draw(path.slice(), 'fill', fillColor.rgba.slice(), lineWidth);
};
context.stroke = function () {
  if (path.length) native.draw(path.slice(), 'stroke', strokeColor.rgba.slice(), lineWidth);
};
function drawRect(args, mode, clear) {
  var p = coordinates(args, 4);
  if (!p) return;
  if (mode === 'fill' ? p[2] === 0 || p[3] === 0 : p[2] === 0 && p[3] === 0) return;
  native.draw([4, p[0], p[1], p[2], p[3]], mode,
