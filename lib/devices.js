...
if (definitions && definitions.buzzer) {
  devices.buzzer = Object.freeze({
    capabilities: definitions.buzzer,
    get state() { return require('mcujs:buzzer').getState(); },
    open: function () {
      if (arguments.length) throw new TypeError('Configured buzzer.open takes no arguments');
      return require('mcujs:buzzer').open();
    }
  });
}
if (definitions && definitions.speaker) {
  devices.speaker = Object.freeze({
    capabilities: definitions.speaker,
    get state() { return require('mcujs:speaker').getState(); },
    open: function () {
      if (arguments.length) throw new TypeError('Configured speaker.open takes no arguments');
      return require('mcujs:speaker').open();
    }
  });
}
if (definitions && definitions.microphone) {
  devices.microphone = Object.freeze({
    capabilities: definitions.microphone,
    get state() { return require('mcujs:microphone').getState(); },
    open: function () {
      if (arguments.length) throw new TypeError('Configured microphone.open takes no arguments');
      return require('mcujs:microphone').open();
    }
  });
}
