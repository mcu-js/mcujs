...
class AbortSignal extends EventTarget {
  // MCU.js cancellation algorithm extension, independent of event propagation.
  static subscribe(signal, callback) {
    var data = signalData(signal);
    if (typeof callback !== 'function') throw new TypeError('Cancellation callback required');
    if (data.aborted) { callback(data.reason); return function () {}; }
    var algorithms = data.algorithms;
    if (algorithms.length >= limits.subscriptions) throw new RangeError('Abort subscription limit exceeded');
    algorithms.push(callback);
    return function () { var index = algorithms.indexOf(callback); if (index >= 0) algorithms.splice(index, 1); };
  }
  constructor(token) {
    if (token !== signalToken) throw new TypeError('Use AbortController to create a signal');
    super();
    get(this, 'target').abort = { aborted: false, reason: undefined, subscriptions: [], algorithms: [] };
  }
...
class AbortController {
  ...
  abort(reason) {
    ...
    algorithms.forEach(function (callback) {
      try { callback(reason); } catch (error) { try { console.error('Cancellation error:', error); } catch (ignored) {} }
    });
