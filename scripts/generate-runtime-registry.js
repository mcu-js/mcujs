...
const { boardDescriptors, manifestFor, shippingBoardIds, configuredDeviceCapabilities, sdDefinitionsFor } = require("../runtime/board-registry.js");
...
const boardMacros = Object.freeze({
  ...
  seeed_reterminal_sticky: "MCUJS_BOARD_SEEED_RETERMINAL_STICKY",
  "waveshare_esp32s3_epaper_1.54_v2": "MCUJS_BOARD_WAVESHARE_ESP32S3_EPAPER_1_54_V2",
});
...
function generateRuntimeRegistryHeader() {
  const branches = Object.keys(boardDescriptors).map((boardId, index) => branchFor(boardId, index === 0));
...
function generateSdConfigHeader() {
  const branches = Object.entries(boardDescriptors).map(([id, descriptor], index) => {
    const lines = [`#${index ? "elif" : "if"} defined(${boardMacros[id]})`];
    for (const [name, value] of Object.entries(sdDefinitionsFor(descriptor))) {
...
function generateSdConfigCmake() {
  const branches = Object.entries(boardDescriptors).map(([id, descriptor], index) =>
    `${index ? "elseif" : "if"}(MCUJS_BOARD_NAME STREQUAL "${id}")\n    set(MCUJS_HAS_SD ${sdDefinitionsFor(descriptor).MCUJS_HAS_SD ? "ON" : "OFF"})\n    add_compile_definitions(MCUJS_HAS_SD=${sdDefinitionsFor(descriptor).MCUJS_HAS_SD} MCUJS_USB_SD_MSC=${sdDefinitionsFor(descriptor).MCUJS_USB_SD_MSC})`);
  return `# Generated by scripts/generate-runtime-registry.js. Do not edit.\n${branches.join("\n")}\nelse()\n    message(FATAL_ERROR "Unknown SD board configuration: \${MCUJS_BOARD_NAME}")\nendif()\n`;
}

const sdGeneratedFiles = {
  "src/generated/sd_config.h": generateSdConfigHeader,
  "src/generated/sd_config.cmake": generateSdConfigCmake,
};
...
\`${configuredDeviceCapabilities.display.interface}\`, at most
${configuredDeviceCapabilities.display.maxOpenHandles} open configured handle.
...
function checkGeneratedFiles() {
  const failures = [];
  for (const [path, generate] of Object.entries(sdGeneratedFiles)) {
    if (readFileSync(join(root, path), "utf8") !== generate()) failures.push(path);
  }
  if (readFileSync(join(root, "runtime/device-capabilities.json"), "utf8") !== JSON.stringify(configuredDeviceCapabilities, null, 2) + "\n") failures.push("runtime/device-capabilities.json");
  ...
  for (const boardId of Object.keys(boardDescriptors)) {
...
function writeGeneratedFiles() {
  for (const [path, generate] of Object.entries(sdGeneratedFiles)) {
    ...
    writeFileSync(join(root, path), generate());
  }
  const header = join(root, generatedHeaderPath);
  mkdirSync(dirname(header), { recursive: true });
  mkdirSync(manifestDirectory, { recursive: true });
  writeFileSync(header, generateRuntimeRegistryHeader());
  writeFileSync(join(root, "runtime/device-capabilities.json"), JSON.stringify(configuredDeviceCapabilities, null, 2) + "\n");
  for (const boardId of Object.keys(boardDescriptors)) {
...
module.exports = {
  generateSdConfigHeader,
  generateSdConfigCmake,
  boardDocsPath,
  checkGeneratedFiles,
