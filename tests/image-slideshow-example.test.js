...
function run(paths,options={}){
 ...
 vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../examples/portable/images/slideshow.js'),'utf8'),{module,console:{log(x){r.events.push(x);}},setTimeout(fn,ms){if(++id===options.failTimer)throw Error('timer failed');timers.set(id,{fn,at:now+ms});return id;},clearTimeout(i){if(options.failClear)throw Error('cancel failure');timers.delete(i);},require(name){
...
test('metadata, open and asynchronous decode errors stop without starting another slide',()=>{
 for(const kind of ['infoError','openError','decodeError']){const r=run(['/app/a.jpg','/app/b.bmp'],{[kind]:'/app/a.jpg'});r.advance(0);if(kind==='decodeError'){r.sessions[0].state='error';r.advance(100);}assert.ok(r.sessions.every(s=>s.file==='/app/a.jpg'));assert.ok(!r.events.includes('SLIDESHOW_DONE'));assert.ok(r.events.some(e=>e.startsWith('SLIDESHOW_ERROR ')));assert.ok(r.events.some(e=>e.startsWith('SLIDE_ERROR ')));r.handle.close();assert.ok(r.sessions.every(s=>s.closed===1));assert.equal(r.pending(),0);}
});
test('a stalled renderer has a per-slide deadline and no endless retry',()=>{const r=run(['/app/a.jpg']);r.advance(122000);assert.equal(r.sessions[0].closed,1);assert.equal(r.pending(),0);assert.ok(r.events.some(e=>e.includes('timed out')));assert.ok(!r.events.includes('SLIDESHOW_DONE'));assert.ok(r.events.some(e=>e.startsWith('SLIDESHOW_ERROR ')));});
...
test('slideshow cancellation attempts active close even if timer removal fails',()=>{
 const r=run(['/app/a.jpg','/app/b.bmp'],{failClear:true});r.advance(0);
 assert.throws(()=>r.handle.close(),/cancel failure/);assert.equal(r.sessions[0].closed,1);
 r.advance(200000);assert.equal(r.sessions.length,1);assert.equal(r.sessions[0].closed,1);
});
