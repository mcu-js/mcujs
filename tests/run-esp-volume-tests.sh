...
(tmp/'esp-path-state.inc').write_text(enum+'static atomic_int s_sd_state = STORAGE_UNINITIALIZED;\n')
...
"$TMP/volumes"
for policy in none ro rw rwlease; do
 flags=""
 case "$policy" in ro) flags="-DMCUJS_HAS_SD=1 -DMCUJS_SD_READONLY=1";; rw) flags="-DMCUJS_HAS_SD=1 -DMCUJS_SD_READONLY=0";; rwlease) flags="-DMCUJS_HAS_SD=1 -DMCUJS_SD_READONLY=0 -DMCUJS_USB_SD_MSC=1";; esac
