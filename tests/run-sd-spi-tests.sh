...
for POLICY in writable readonly; do
    POLICY_FLAG=""
    if [ "$POLICY" = readonly ]; then POLICY_FLAG="-DMCUJS_SD_READONLY=1"; fi
    EXTRA=""
    if [ "$BITS" = 64 ]; then EXTRA="-DSD_TEST_LBA64=1"; fi
    "${CC:-cc}" -std=c11 -Wall -Wextra -Werror -pedantic -O1 -g \
        ${SD_TEST_CFLAGS:-} $EXTRA $POLICY_FLAG -DSD_TEST_BOARD="$BOARD" -DMCUJS_SD_SPI_BAUD_HZ="$RATE" \
        ...
        -o "$TMP/sd-spi-$BITS"
    printf 'Board %s SPI target %s Hz, %s: ' "$BOARD" "$RATE" "$POLICY"
    "$TMP/sd-spi-$BITS"
done
done
done
